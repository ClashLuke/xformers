...
def kernel_fma(
    ...
    **META,
    ...
    if META["SAVE_ACT_INPUTS"]:
        act_inputs_ptrs = ACT_INPUTS + rm[:, None] * stride_aim + rn[None, :]
        tl.store(act_inputs_ptrs, acc, mask=(rm[:, None] < M) & (rn[None, :] < N))
...
def fused_matmul_backward(
    ...
    if trainable_bias and trainable_weight:
        ...
        def grid(META):
            return (
                triton.cdiv(N, META["BLOCK_ROW"]) * triton.cdiv(K, META["BLOCK_COL"]),
            )
